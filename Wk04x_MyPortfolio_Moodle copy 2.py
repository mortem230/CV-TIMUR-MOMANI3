...
def _(count, filtered_portfolio, mo, pd, px):
    ...
    travel_data = pd.DataFrame({
        'City': ['London', 'New York', 'Tokyo', 'Sydney', 'Paris', 'Almaty', 'Dubai','Istanbul'],
        'Lat': [51.5, 40.7, 35.6, -33.8, 48.8, 43.2, 25.2, 41.0],
        'Lon': [-0.1, -74.0, 139.6, 151.2, 2.3, 76.9, 55.3, 28.9],
        'Visit_Year_str': ['2022', '2023', '2024', '2021', '2023', '2006-2019', '2023','2019-2025']
    })
...
app._unparsable_cell(
    ...
    tab_cv = mo.md(
        \"\"\"
        ### Aspiring Financial Analyst | Data Enthusiast | Market Explorer
        ...
        *   *Relevant Modules:* Introduction to Data Science and AI Tools, Financial Accounting.
        ...
        tech_md = mo.md(
            ...
            \" \".join([
                ...
                '<span class=\"badge\">Data Handling</span>',
                '<span class=\"badge\">Financial Documentation</span>',
                '<span class=\"badge\">Social Media Coordination</span>',
    ...
    tab_data_content = mo.vstack([
        ...
        sample_sp500 = pd.DataFrame({
            ...
            \"Sector\": [\"Technology\", \"Technology\", \"Communication Services\", \"Consumer Cyclical\",
                       \"Technology\", \"Communication Services\", \"Consumer Cyclical\", \"Financial Services\",
                       \"Financial Services\", \"Healthcare\"],
            \"Market Cap (B)\": [2850, 3100, 1950, 1800, 2200, 1250, 550, 480, 520, 380],
            \"Revenue (B)\": [383, 227, 307, 574, 60, 134, 97, 158, 35, 85]
        })
    
        min_cap = int(sample_sp500[\"Market Cap (B)\"].min())
        max_cap = int(sample_sp500[\"Market Cap (B)\"].max())
        cap_slider = mo.ui.slider(min_cap, max_cap, value=min_cap, label=\"Minimum Market Cap (Billions USD)\")
    
        data_demo = mo.vstack([
            mo.md(\"## 📊 Data Literacy Showcase: S&P 500 Sample\"),
            mo.md(\"### Filter Companies by Minimum Market Cap\"),
            cap_slider,
            mo.ui.table(sample_sp500[sample_sp500[\"Market Cap (B)\"] >= cap_slider.value], selection=None)
        ])
        return cap_slider, data_demo, max_cap, min_cap, sample_sp500
    ...
    tab_personal = mo.vstack([
       def__(mo):
        # Passion Projects Tab
        passion_md = mo.md(
            \"\"\"
            ## 🏆 Passion Projects
        
            <div class=\"project-card\">
            <h3>🚗 Drift Car Build: Nissan S2000 → 1500 HP Monster</h3>
            Modified a Nissan S2000 into a manual drift champion pushing **1500 horsepower**. 
            Complete engine rebuild, custom turbo setup, and suspension tuning for competitive drifting.
            </div>
        
            <div class=\"project-card\">
            <h3>📈 Silver Trading Portfolio</h3>
            Grew an initial investment of **$1,000 to $230,000** in under 15 months. 
            Averaged **$10,000–$15,000 monthly profit** through disciplined technical analysis and risk management.
            </div>
        
            <div class=\"project-card\">
            <h3>🥊 Professional Boxing Champion</h3>
            **Light Heavyweight Division** – Competed and won at professional level. 
            Training discipline translates directly to focus and resilience in all areas of life.
            </div>
    """,
    name="_"
)
...
app._unparsable_cell(
    ...
    app_tabs = mo.ui.tabs({
        \"📄 About Me\":tab_cv  # ty:ignore[unresolved-reference],
        \"📊 Passion Projects\": tab_data_content  # ty:ignore[unresolved-reference] 
        \"✈️ Personal Interests\": tab_personal  # ty:ignore[unresolved-reference]
        })
